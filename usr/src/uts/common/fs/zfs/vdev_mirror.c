 ...
 * the elevator will be able to merge requests while it is plugged.
 * Otherwise, requests are queued to the least busy device.
 *
 * For rotational disks the elevator will plug for 10ms which is why
 * zfs_vdev_mirror_switch_us is set to 10ms by default. For non-rotational
 * disks the elevator will not plug, but 10ms is still a small enough value
 * that the requests will get spread over all the children.
 *
...
vdev_mirror_map_alloc(zio_t *zio)
{
	...
	if (vd == NULL) {
		...
		d = gethrtime() / (1000 * zfs_vdev_mirror_switch_us);
		d = (d % lowest_nr) + 1;
