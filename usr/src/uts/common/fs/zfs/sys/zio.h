 ...
 * is not `off', it will call the `sc_ask' callback function, asking the
 * upper layers whether it should really try to compress the object in
 * question. If the function returns B_TRUE, the zio is compressed. After
 * compression is done, sc_result is called to inform the upper layers of
 * the compression result. By comparing the zio's io_size to io_orig_size
 * it can monitor compression performance on the particular object in
 * question (if io_size == io_orig_size, then compression failed).
 */
