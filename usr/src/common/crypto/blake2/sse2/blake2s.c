...
static inline int
blake2s_clear_lastnode(blake2s_state * S)
{
	S->f[1] = 0U;
	return (0);
}
...
static inline int
blake2s_clear_lastblock(blake2s_state * S)
{
	if (S->last_node)
...
static inline int
blake2s_param_set_digest_length(blake2s_param * P, const uint8_t digest_length)
{
...
static inline int
blake2s_param_set_fanout(blake2s_param * P, const uint8_t fanout)
{
	P->fanout = fanout;
	return (0);
}
...
static inline int
blake2s_param_set_max_depth(blake2s_param * P, const uint8_t depth)
{
	P->depth = depth;
	return (0);
}
...
static inline int
blake2s_param_set_leaf_length(blake2s_param * P, const uint32_t leaf_length)
{
...
static inline int
blake2s_param_set_node_offset(blake2s_param * P, const uint64_t node_offset)
{
...
static inline int
blake2s_param_set_node_depth(blake2s_param * P, const uint8_t node_depth)
{
...
static inline int
blake2s_param_set_inner_length(blake2s_param * P, const uint8_t inner_length)
{
...
static inline int
blake2s_param_set_salt(blake2s_param * P, const uint8_t salt[BLAKE2S_SALTBYTES])
{
 ...
 */
int
blake2s_init(blake2s_state * S, const uint8_t outlen)
{
...
int
blake2s_init_key(blake2s_state * S, const uint8_t outlen, const void *key,
...
static inline int
blake2s_compress(blake2s_state * S, const uint8_t block[BLAKE2S_BLOCKBYTES])
{
	...
	row4 =
	    _mm_xor_si128(_mm_setr_epi32(0x510E527F, 0x9B05688C, 0x1F83D9AB,
		0x5BE0CD19), LOAD(&S->t[0]));
	ROUND(0);
	ROUND(1);
	ROUND(2);
	ROUND(3);
...
/* inlen now in bytes */
int
blake2s_update(blake2s_state * S, const uint8_t * in, uint64_t inlen)
{
	while (inlen > 0) {
...
/* Is this correct? */
int
blake2s_final(blake2s_state * S, uint8_t * out, uint8_t outlen)
{
...
int
blake2s(uint8_t * out, const void *in, const void *key, const uint8_t outlen,
...
main(int argc, char **argv)
{
	...
	for (size_t i = 0; i < KAT_LENGTH; ++i) {
		...
		if (blake2s(hash, buf, key, BLAKE2S_OUTBYTES, i,
			BLAKE2S_KEYBYTES) < 0 ||
