...
static inline int
blake2sp_init_leaf(blake2s_state * S, uint8_t outlen, uint8_t keylen,
    uint64_t offset)
{
	blake2s_param P[1];
...
static inline int
blake2sp_init_root(blake2s_state * S, uint8_t outlen, uint8_t keylen)
{
	blake2s_param P[1];
	P->digest_length = outlen;
	P->key_length = keylen;
...
int
blake2sp_init(blake2sp_state * S, const uint8_t outlen)
{
...
int
blake2sp_init_key(blake2sp_state * S, const uint8_t outlen, const void *key,
...
int
blake2sp_update(blake2sp_state * S, const uint8_t * in, uint64_t inlen)
{
...
int
blake2sp_final(blake2sp_state * S, uint8_t * out, const uint8_t outlen)
{
...
int
blake2sp(uint8_t * out, const void *in, const void *key, uint8_t outlen,
...
#pragma omp parallel shared(S, hash), num_threads(PARALLELISM_DEGREE)
...
#endif
	{
		...
		if (inlen__ > id__ * BLAKE2S_BLOCKBYTES) {
			...
			const size_t len =
			    left <=
			    BLAKE2S_BLOCKBYTES ? left : BLAKE2S_BLOCKBYTES;
...
main(int argc, char **argv)
{
	...
	for (size_t i = 0; i < KAT_LENGTH; ++i) {
		...
		if (0 != memcmp(hash, blake2sp_keyed_kat[i],
		    BLAKE2S_OUTBYTES)) {
			puts("error");
			return (-1);
		}
